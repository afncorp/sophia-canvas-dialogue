...
export const LoanProgramsTabs = ({ onAskSophia }: LoanProgramsTabsProps) => {
  ...
  const loanPrograms: LoanProgram[] = [
    {
      ...
      keyFeatures: [
        ...
        "Loan amounts up to conforming limits ($832,750)",
  ...
  useEffect(() => {
    ...
    const index = loanPrograms.findIndex(p => p.id === activeTab);
    if (index >= 0) {
      carouselApi.scrollTo(index);
    }
  }, [activeTab, carouselApi]);

  // Sync tab with carousel on mobile
  useEffect(() => {
    if (!carouselApi) return;
    const onSelect = () => {
      const index = carouselApi.selectedScrollSnap();
      setActiveTab(loanPrograms[index].id);
    };
          ...
          {loanPrograms.map((program) => {
            ...
            return (
              <TabsTrigger
                key={program.id}
                value={program.id}
                className="text-xs md:text-sm px-1 md:px-3 data-[state=active]:bg-gradient-to-r data-[state=active]:from-primary/20 data-[state=active]:to-secondary/20 data-[state=active]:text-foreground data-[state=active]:shadow-lg transition-all"
              >
            ...
            <CarouselContent>
              {loanPrograms.map((program) => (
                <CarouselItem key={program.id}>
                  <div className="relative bg-card/40 backdrop-blur-md rounded-2xl p-6 border-2 border-primary/40 shadow-xl shadow-primary/20">
                    {/* Program Header */}
